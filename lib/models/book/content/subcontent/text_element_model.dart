...
typedef TextElements = List<TextElementModel>;
...
class TextElementModel {
  ...
  TextElementModel.fromTxt(String txt,
  ...
  TextElementModel.fromXml(XmlElement xml,
      {Attrs? attributes, DisplayType? parentDisplayType, DisplayType? type}) {
    ...
    if (hasChildren) {
      ...
      for (var child in xml.children) {
        if (child.nodeType == XmlNodeType.ELEMENT) {
          subelements.add(TextElementModel.fromXml(childNodes.removeAt(0),
              parentDisplayType: displayType));
        } else {
          subelements.add(TextElementModel.fromTxt(child.text,
              parentDisplayType: displayType));
        }
      }
    }
  ...
  Attrs _getAttrs(XmlElement xml, DisplayType displayType) {
    ...
    if (displayType == DisplayType.bookref) {
      ...
      attrs['href'] =
          'https://www.projectaon.org/en/xhtml/$series/$book/title.htm';
    }
  ...
  Json toJson() => {
        ...
        'subelements':
            subelements.map((subelement) => subelement.toJson()).toList(),
        'text': text,
      };
